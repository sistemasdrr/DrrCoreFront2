...
export class ListaEmpresas2Component implements OnInit {
  ...
  constructor(private antecedentesService : AntecedentesLegalesService,@Inject(MAT_DIALOG_DATA) public data: any,private datosEmpresaService : DatosEmpresaService,private router : Router, private paisService : PaisService,public dialogRef: MatDialogRef<ListaEmpresas2Component>,){
  ...
  filtrarEmpresas(){
    ...
    this.datosEmpresaService.getDatosEmpresas(this.razonSocial.trim(), this.filtroRB, this.idPais, this.chkConInforme,"N",'T').subscribe(
      (response) => {
