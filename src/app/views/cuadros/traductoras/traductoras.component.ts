...
export class TraductorasComponent implements OnInit {
  ...
  descargarDocumento(idQuery: number, format: string) {
    switch (idQuery) {
      ...
      case 1:
        ...
        this.loading = true;
        this.reportService
          .DownloadReport6_5_3(this.query1_3_startString, this.query1_3_endString, this.query1_3_code, format)
          .subscribe((response) => {
            const blob: Blob = response.body as Blob;
            const a = document.createElement('a');
            a.download =
  ...
  selectQuery6_5_3(){
    this.loading = true
    this.reportService
    .DownloadReport6_5_3(this.query1_3_startString, this.query1_3_endString,this.query1_3_code, 'pdf')
    .subscribe((response) => {
